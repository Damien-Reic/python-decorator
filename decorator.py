...
def decorator_with_log(func):
    ...
    def wrapper(*args, **kwargs):
        with open(getcwd() + '/' + "logs.txt", "a") as f:     # open the log file
            start = (datetime.now(),time())                    # defines the time at which the program starts to run
            print("started")
            value = func(*args, **kwargs)               #get the func result
            f.write(f"Function '{func.__name__}' called with {[arg for arg in args]} at {start[0]}  return {value} and done in {time() - start[1]} seconds" + "\n") #write the informations in the logs.txt file
            print("done")
